...
class ImageContentHelper:
    ...
    # 그 자리에 무엇이 있었는지 한 줄로 알려 준다 (사용자에게는 보이지 않고 모델 프롬프트에만 들어간다)
    ...
    @staticmethod
    def strip_image_block(message_content : Any) -> Any:
        ...
        if last_text_index is None:
            # 텍스트가 아예 없던 경우(이미지만 보낸 질문)는 안내 문구만 남긴다
        ...
        kept_block_list[last_text_index] = {**last_text_block,
                                            "text" : f"{last_text_block.get('text', '')}\n{ImageContentHelper.REMOVED_IMAGE_NOTICE_TEXT}".strip()}
