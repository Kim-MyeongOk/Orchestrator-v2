...
class RedisChunkBuffer:
    ...
    @staticmethod
    def _create_chunk_list_key(thread_id : uuid.UUID, run_id : uuid.UUID) -> str:
        # {thread_id} 해시태그 : Redis Cluster 에서 같은 스레드의 키가 동일 슬롯에 놓이게 한다
        return f"orch:{{{thread_id}}}:run:{run_id}:chunk_list"

    async def append_chunk_async(self, thread_id : uuid.UUID, run_id : uuid.UUID, chunk_dictionary : Dict[str, Any]) -> None:
    ...
    async def get_chunk_dictionary_list_async(self, thread_id : uuid.UUID, run_id : uuid.UUID) -> List[Dict[str, Any]]:
        chunk_list_key  = RedisChunkBuffer._create_chunk_list_key(thread_id, run_id)
    ...
    async def delete_buffer_async(self, thread_id : uuid.UUID, run_id : uuid.UUID) -> None:
